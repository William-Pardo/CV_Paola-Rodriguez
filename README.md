<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Currículum Vitae - Paola Andrea Rodríguez Cárdenas</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;600;700&display=swap" rel="stylesheet">
    <style>
        body {
            font-family: 'Inter', sans-serif;
            background-color: #f3f4f6;
        }
    </style>
</head>
<body class="bg-gray-100 p-4 sm:p-8">
    <div class="max-w-4xl mx-auto bg-white rounded-xl shadow-lg overflow-hidden">
        
        <!-- Header Section -->
        <div class="bg-gradient-to-r from-blue-600 to-cyan-500 text-white p-8 sm:p-12 text-center rounded-t-xl">
            <h1 class="text-3xl sm:text-4xl font-bold tracking-tight mb-2">PAOLA ANDREA RODRÍGUEZ CÁRDENAS</h1>
            <p class="text-lg sm:text-xl font-medium opacity-90">Enfermera Especialista en Salud Pública</p>
        </div>

        <!-- Main Content -->
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8 p-6 sm:p-10">
            
            <!-- Left Column: Sidebar -->
            <div class="md:col-span-1 space-y-8">
                
                <!-- Contact Information -->
                <div class="bg-gray-50 p-6 rounded-lg shadow-inner">
                    <h2 class="text-xl font-bold text-gray-800 mb-4 border-b-2 border-blue-200 pb-2">Información de Contacto</h2>
                    <ul class="text-gray-700 space-y-3">
                        <li><span class="font-semibold text-gray-900">Teléfono:</span> 3197166549</li>
                        <li><span class="font-semibold text-gray-900">Correo:</span> gengepardo@gmail.com</li>
                        <li><span class="font-semibold text-gray-900">Ubicación:</span> Bogotá, Colombia</li>
                        <li>
                            <a href="https://linkedin.com/in/paolarodriguez" target="_blank" class="text-blue-600 hover:text-blue-800 transition-colors duration-200 font-medium">
                                Ver perfil en LinkedIn
                            </a>
                        </li>
                    </ul>
                </div>
                
                <!-- Education -->
                <div class="bg-gray-50 p-6 rounded-lg shadow-inner">
                    <h2 class="text-xl font-bold text-gray-800 mb-4 border-b-2 border-blue-200 pb-2">Educación</h2>
                    <ul class="text-gray-700 space-y-4">
                        <li>
                            <p class="font-semibold">Especialista en Salud Pública</p>
                            <p class="text-sm italic">Fundación Universitaria Sanitas (Nov. 2019)</p>
                        </li>
                        <li>
                            <p class="font-semibold">Enfermera Superior</p>
                            <p class="text-sm italic">Universidad de Ciencias Aplicadas y Ambientales (U.D.C.A) (2009)</p>
                        </li>
                    </ul>
                </div>

                <!-- Skills -->
                <div class="bg-gray-50 p-6 rounded-lg shadow-inner">
                    <h2 class="text-xl font-bold text-gray-800 mb-4 border-b-2 border-blue-200 pb-2">Habilidades</h2>
                    <ul class="list-disc list-inside space-y-2 text-gray-700">
                        <li>Liderazgo de equipos</li>
                        <li>Gestión de proyectos de salud</li>
                        <li>Comunicación efectiva</li>
                        <li>Control y vigilancia en salud pública</li>
                        <li>Soporte Vital Básico (BLS)</li>
                        <li>Procedimientos de citología y planificación familiar</li>
                    </ul>
                </div>

                <!-- Non-formal courses -->
                <div class="bg-gray-50 p-6 rounded-lg shadow-inner">
                    <h2 class="text-xl font-bold text-gray-800 mb-4 border-b-2 border-blue-200 pb-2">Cursos</h2>
                    <ul class="list-disc list-inside space-y-2 text-gray-700">
                        <li>Soporte Vital Básico RCP (BLS)</li>
                        <li>Actualización en VPH y Cáncer de Cuello Uterino</li>
                        <li>Inserción y Remoción de Implantes Subdérmicos</li>
                        <li>Asesoría para VIH/SIDA</li>
                    </ul>
                </div>

            </div>

            <!-- Right Column: Main Sections -->
            <div class="md:col-span-2 space-y-8">
                
                <!-- Professional Summary -->
                <div class="p-6 bg-blue-50 rounded-lg shadow-md">
                    <h2 class="text-2xl font-bold text-blue-800 mb-4">Perfil Profesional</h2>
                    <p class="text-gray-700 leading-relaxed">
                        Enfermera especialista en salud pública con más de 11 años de experiencia, enfocada en la **promoción, prevención y el liderazgo de proyectos de salud**. Demuestro un historial comprobado en la creación y ejecución de programas de alto impacto que han mejorado la salud de la comunidad. Mi enfoque se basa en el **liderazgo proactivo, la responsabilidad y la honestidad** para lograr resultados excelentes y de alta calidad.
                    </p>
                </div>
                
                <!-- Key Achievements -->
                <div class="p-6 bg-white rounded-lg shadow-md border-t-4 border-blue-500">
                    <h2 class="text-2xl font-bold text-gray-800 mb-4">Logros y Experiencia Clave</h2>
                    <ul class="space-y-6 text-gray-700">
                        <li>
                            <span class="font-semibold text-lg text-blue-600">Liderazgo de Proyectos Estratégicos:</span> Lideré el programa de Salud de la Mujer en Compensar, obteniendo reconocimientos por el impacto positivo. Fui responsable de la ejecución exitosa de la prueba piloto de genotipificación de VPH, un proyecto innovador que mejoró significativamente la detección y prevención del cáncer.
                        </li>
                        <li>
                            <span class="font-semibold text-lg text-blue-600">Planificación Familiar:</span> Lideré el programa, realizando procedimientos clave como citología cérvico-vaginal, inserción y extracción de implantes subdérmicos y DIU, demostrando alta competencia técnica y atención al paciente.
                        </li>
                        <li>
                            <span class="font-semibold text-lg text-blue-600">Vigilancia en Salud Pública:</span> Coordiné el control y la vigilancia de eventos de interés, liderando auditorías y procesos de retroalimentación para la mejora continua de la calidad asistencial en el Hospital.
                        </li>
                    </ul>
                </div>
                
                <!-- Professional Experience -->
                <div class="p-6 bg-white rounded-lg shadow-md border-t-4 border-blue-500">
                    <h2 class="text-2xl font-bold text-gray-800 mb-4">Experiencia Profesional</h2>
                    <div class="space-y-6">
                        <div class="border-b pb-4">
                            <p class="font-semibold text-lg">Líder en Citologías</p>
                            <p class="italic text-gray-600">Viva 1A (Junio 2025 - Septiembre 2025)</p>
                            <p class="text-sm mt-1 text-gray-700">Gestión y optimización de procesos de citologías.</p>
                        </div>
                        <div class="border-b pb-4">
                            <p class="font-semibold text-lg">Enfermera de Salud Pública</p>
                            <p class="italic text-gray-600">Sociedad de Cirugía de Bogotá - Hospital de San José (Abril 2024 - Presente)</p>
                        </div>
                        <div class="border-b pb-4">
                            <p class="font-semibold text-lg">Enfermera Asistencial</p>
                            <p class="italic text-gray-600">Compensar (Mayo 2012 - Agosto 2022)</p>
                            <p class="text-sm mt-1 text-gray-700">Enfocada en la implementación y liderazgo de proyectos de prevención y promoción.</p>
                        </div>
                        <div class="border-b pb-4">
                            <p class="font-semibold text-lg">Enfermera de Prevención y Promoción</p>
                            <p class="italic text-gray-600">Unidad Médica Santa Fe - Compensar (Abril 2011 - Abril 2012)</p>
                        </div>
                        <div>
                            <p class="font-semibold text-lg">Enfermera Jefe y Líder de Brigadas</p>
                            <p class="italic text-gray-600">Sanidad de la Policía Nacional (Enero 2010 - Enero 2011)</p>
                        </div>
                    </div>
                </div>

            </div>
        </div>
        
    </div>
</body>
</html>
